...
uint8_t *pui8buffer;
    ...
    if( USBD_OK == UsbReceive(&pui8buffer, &rxLen) )
    {/*
	   ...
	   * packets (up to max. 256 u8s)		   */
		uint8_t payload  = USB_HID_PAYLOAD_SIZE( pui8buffer );
		if ( USB_HID_STATUS( pui8buffer ) != 0 )
...
uint16_t offset = 0;
	...
	while ( totalTxSize > 0 )
	{
		...
		offset += payload;
		/* initiate transfer now */
		UsbTransmit(usbTxBuffer, payload + FRAME_HEADER_SIZE);
		ioLedOff( );
	}
}
