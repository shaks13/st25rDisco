...
/*******************************************************************************/
ReturnCode rfalNfcvPollerWriteMultipleBlock( uint8_t flags, const uint8_t* uid, uint8_t firstBlockNum, uint8_t numOfBlocks, uint8_t *txBuf, uint16_t txBufLen, uint8_t blockLen, const uint8_t* wrData, uint16_t wrDataLen )
{
    ReturnCode         ret;
    uint16_t           rcvLen;
    uint16_t           reqLen;
    ...
    if( (reqLen > txBufLen) || (blockLen > (uint8_t)RFAL_NFCV_MAX_BLOCK_LEN) || ((((uint16_t)numOfBlocks + 1U) * (uint16_t)blockLen) != wrDataLen) )
    {
        return ERR_PARAM;
    }
...
/*******************************************************************************/
ReturnCode rfalNfcvPollerExtendedWriteMultipleBlock( uint8_t flags, const uint8_t* uid, uint16_t firstBlockNum, uint16_t numOfBlocks, uint8_t *txBuf, uint16_t txBufLen, uint8_t blockLen, const uint8_t* wrData, uint16_t wrDataLen )
{
    ReturnCode         ret;
    uint16_t           rcvLen;
    uint16_t           reqLen;
    ...
    if( (reqLen > txBufLen) || (blockLen > (uint8_t)RFAL_NFCV_MAX_BLOCK_LEN) || (((numOfBlocks + 1U) * (uint16_t)blockLen) != wrDataLen) )
    {
        return ERR_PARAM;
    }
    ...
    txBuf[msgIt++] = (uint8_t)((numOfBlocks >> 0) & 0xFFU);
    txBuf[msgIt++] = (uint8_t)((numOfBlocks >> 8) & 0xFFU);
